...
import {
  ...
  AlertTriangle, Download, Filter, RefreshCw, Star
} from "lucide-react";
import {
...
const USERS: { id: number; name: string; email: string; plan: string; joined: string; status: string; score: number }[] = [
...
const QUESTIONS: { id: number; text: string; section: string; difficulty: string; status: string; reports: number }[] = [
...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      {/* Top bar */}
      <div className="border-b border-white/10 bg-gray-950/80 backdrop-blur-xl sticky top-0 z-30 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">CATPrep.ai</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-gray-400 text-sm hover:text-white transition-all">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {STATS.map((s, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
                  className={cn("bg-white/5 border rounded-2xl p-5", COLOR_MAP[s.color].split(" ").slice(2).join(" "))}>
                  <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center mb-3 border", COLOR_MAP[s.color])}>
        ...
        {tab === "questions" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1 bg-white/5 border border-white/10 rounded-xl p-1">
                {["All", "QA", "VARC", "DILR"].map((s) => (
                  <button key={s} onClick={() => setQSection(s)}
                    className={cn(
            ...
            <div className="space-y-2">
              {filteredQ.map((q) => (
                <div key={q.id} className="bg-white/5 border border-white/10 rounded-xl px-5 py-4 flex items-center justify-between gap-4 hover:bg-white/[0.07] transition-all">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {q.reports > 0 && (
                      <span className="flex items-center gap-1 text-xs text-yellow-400 mr-2">
                        <Star className="w-3 h-3" /> {q.reports} reports
                      </span>
                    )}
